...
describe("USDC Pool", function () {
    ...
    beforeEach(async function () {
        ...
        fakeEuro = await FakeUSDC.deploy("Fake EUR", "fEUR", 10000000000000);
        ...
        const UniswapV3SwapRouterMock = await hre.ethers.getContractFactory("UniswapV3SwapRouterMock");
        swapRouter = await UniswapV3SwapRouterMock.deploy(fakeUSDC.address, fakeEuro.address);
        ...
        const euroRamp = await Ramp.deploy(
            fakeEuro.address,
            swapRouter.address,
            quoter,
            usdcEuroPoolFee,
        ...
        await ramp.connect(offRamper).updateEuroPool(euroRamp.address);
    ...
    describe("completeCrossChainOrder", function () {
        ...
        it("should complete an exchange using mock uniswap", async function () {

            await ramp.connect(onRamper).completeCrossChainOrder(
                [
                    a,
                    b,
                    c
