...
contract EUROPool is IPool {
  ...
  function completeCrossChainOrder(
    ...
    uint256 euroAmount = completeOnRampOrder(
      proof,
      inputs,
  ...
  function completeOnRampOrder(
    ...
    uint256[31] memory inputs,
    string memory xBorderRecieverAddress
  ) public returns (uint256 euroAmount) {
    (uint256 orderId, uint256 euroAmount) = _verifyOnRampOrder(
      proof,
      inputs,
      xBorderRecieverAddress
    );
  ...
  function _verifyOnRampOrder(
    ...
    uint256[31] memory inputs,
    string memory xBorderRecieverAddress
  ) internal returns (uint256 orderId, uint256 euroAmount) {
    // Verify that proof generated by onRamper is valid
    (
